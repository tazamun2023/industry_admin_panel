      ...
      <template v-if="approvedModal">
        <div class="fixed bg-modal  inset-0 z-50 flex items-center justify-center">
          ...
          <div class="z-50 bg-white p-6 relative rounded-md shadow w-full md:w-1/2 lg:w-2/3 xl:w-1/5">
            ...
            <!-- Modal Content -->
            <div class="mb-4">
              <h4>{{ $t('vendor.verify_message') }}</h4>
            </div>
            <!-- Close Button -->
            <div class="flex justify-end gap-4">
              <button @click="approvedModal = false"
                      class="p-2 border text-center rounded border-primary w-[50px] leading-3">No
              </button>
              <button @click="approval(value.id, 1)"
                      class="p-2 border border-primary bg-primary text-center rounded text-white w-[50px] leading-3">Yes
              </button>
            </div>
    ...
    async approval(val, status) {
      ...
      if (app) {
        const data = await this.changeVendorStatus({
          params: {'vendor_id': val, 'verified': status},
