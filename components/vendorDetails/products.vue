<template>
  <div>
    <list-page
      :filter="false"
      ref="listPage"
      list-api="getVendorProduct"
      delete-api="deleteVendorProduct"
      route-name="vendors"
      :param="param"
    ...
    <table>
      ...
      <tr v-for="(value, index) in list" :key="index">
        ...
        <td v-if="value.image.length == 0"><img src="http://127.0.0.1:8000/uploads/thumb-default-image.webp" alt=""></td>
        <td v-else>{{ value.image }}</td>
        <td>{{ value.title }}</td>
        <td>{{ value.sku }}</td>
        <td>{{ value.category }}</td>
        <td>{{ value.product_prices }}</td>
