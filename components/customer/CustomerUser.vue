        ...
        <DeleteModal v-if="deleteModal" @closeModal="closeModal">
          ...
          <template v-slot:buttons>
            <div class="flex gap-4 justify-end">
              <button @click="deleteModal=false" class="p-2 border border-smooth rounded leading-3 w-[60px]">{{ $t('customer.Quit') }}</button>
              <button @click="$refs.listPage.deleteContentItem(value.id)" class="p-2 border border-smooth bg-primary text-white  rounded leading-3 w-[60px] hover:text-primary">
                {{ $t('customer.Agree') }}</button>
            </div>
          </template>
        </DeleteModal>
      </tr>
