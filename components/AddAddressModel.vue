          ...
          <ValidationObserver  v-slot="{ invalid }">
            <form @submit.prevent="addressAction">
                    ...
                    <div class="input-wrapper w-full">
                       ...
                       <div class="flex gap-2">
                         <ValidationProvider tag="div" name="Phone Code" class="w-full phone-code" rules="required"
                          v-slot="{ errors }" :custom-messages="{required: `Enter Your ${$t('address.phone_code')}`}">
                     ...
                     <ValidationProvider tag="div" name="phone" class="w-full" rules="required|numeric" v-slot="{ errors }" :custom-messages="{required: `Enter Your ${$t('address.phone')}`, numeric: 'Enter Number'}">
              ...
              <div class="flex justify-end gap-4">
                <button type="button" @click.prevent="$emit('close')" class="btn bg-smooth hover:text-primary  border-secondary mt-20">{{ $t('address.Cancel') }}</button>
                <button class="btn bg-primary hover:text-primary text-white border-secondary mt-20" :disabled="invalid">{{ $t('address.Save Change') }}</button>
              </div>
            </form>
          </ValidationObserver>
        </div>
