...
export default {
  ...
  methods: {
    ...
    debouncedInputHandler: _.debounce(function () {
      ...
      //variants[openTab]?.result.sku,variants[openTab]?.result.id
      this.checkIfVaildSKU({
        sku: this.variants[this.openTab]?.result.sku,
        product_id: this.variants[this.openTab]?.result.id
      })
